...
void sound_sdl::update_audio_stream(bool is_throttled, const int16_t *buffer, int samples_this_frame)
{
    ...
    if (mSampleRate != 0 && stream_buffer)
	{
        ...
        write_position = stream_playpos + ((mSampleRate / 50) * sizeof(int16_t) * 2);
...
int sound_sdl::init(const int sample_rate)
{
	int         n_channels = 2;
	int         audio_latency;
...
int sound_sdl::sdl_create_buffers(void)
{
    ...
    stream_buffer = (int8_t*)malloc(stream_buffer_size);
	stream_playpos = 0;
	buf_locked = 0;
	return 0;
}
