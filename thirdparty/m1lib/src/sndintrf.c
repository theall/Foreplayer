/* Sound DSP glue and interface routines */
...
int sound_boot(void)
{
	...
	for (i = 0;i < SOUND_COUNT;i++)
	{
        if (sndintf[i].sound_num != (unsigned int)i)
		{
			...
			logerror("Sound #%d %s wrong ID %d: check enum SOUND_... in src/sndintrf.h!\n",i,sndintf[i].name,sndintf[i].sound_num);
	...
	{
//		printf("found sound chip type %d\n", Machine->drv->sound[totalsound].sound_type);
		if ((*sndintf[Machine->drv->sound[totalsound].sound_type].start)(&Machine->drv->sound[totalsound]) != 0)
			goto getout;

//		printf("Started chip OK\n");
		totalsound++;
	}
