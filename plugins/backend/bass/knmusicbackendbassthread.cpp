...
bool KNMusicBackendBassThread::loadTrack(const KNMusicTrackDetailInfo &trackInfo)
{
    //Stop the current thread first.
    stop();

    QString filePath=trackInfo.musicFilePath;
    ...
    if(trackInfo.startPosition!=-1 && trackInfo.startPosition<m_duration)
    {
        //Save the start position.
        m_startPosition=trackInfo.startPosition;
        //Check the validation of the duration.
        if(trackInfo.trackDuration==-1 || trackInfo.startPosition+trackInfo.trackDuration>m_duration)
        {
        ...
        else
        {
            ...
            m_duration=trackInfo.trackDuration;
        }
...
QList<MusicType> KNMusicBackendBassThread::musicTypes() const
{
    return m_musicTypes;
}
