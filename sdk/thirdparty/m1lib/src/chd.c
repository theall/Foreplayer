...
UINT32 chd_get_metadata(struct chd_file *chd, UINT32 *metatag, UINT32 metaindex, void *outputbuf, UINT32 outputlen)
{
	...
	memset(&metaentry, 0, sizeof(metaentry));
...
static int read_hunk_into_memory(struct chd_file *chd, UINT32 _hunknum, UINT8 *dest)
{
	struct map_entry *entry = &chd->map[_hunknum];
	UINT32 bytes;
	int err;
...
static int read_hunk_into_cache(struct chd_file *chd, UINT32 _hunknum)
{
	int err;

	/* if we're already in the cache, we're done */
	if (chd->cachehunk == _hunknum)
		return CHDERR_NONE;
	chd->cachehunk = ~0;
	
	/* otherwise, read the data */
	err = read_hunk_into_memory(chd, _hunknum, chd->cache);
	if (err != CHDERR_NONE)
		return err;
	
	/* mark the hunk successfully cached in */
	chd->cachehunk = _hunknum;
	return CHDERR_NONE;
}
...
static int write_hunk_from_memory(struct chd_file *chd, UINT32 _hunknum, const UINT8 *src)
{
	struct map_entry *entry = &chd->map[_hunknum];
	...
	if (chd->header.compression == CHDCOMPRESSION_ZLIB_PLUS)
	{
		...
		match = find_matching_hunk(chd, _hunknum, newentry.crc, &src[0]);
		if (match != NO_MATCH)
		{
...
write_entry:
	*entry = newentry;
	...
	assemble_map_entry(&fileentry[0], &chd->map[_hunknum]);
	bytes = multi_write(chd->file, chd->header.length + _hunknum * sizeof(fileentry), sizeof(fileentry), &fileentry[0]);
...
static void add_to_crcmap(struct chd_file *chd, UINT32 _hunknum)
{
	UINT32 hash = chd->map[_hunknum].crc % CRCMAP_HASH_SIZE;
...
static int is_really_matching_hunk(struct chd_file *chd, UINT32 _hunknum, const UINT8 *rawdata)
{
	/* we have a potential match -- better be sure */
	/* read the hunk from disk and compare byte-for-byte */
	if (_hunknum != chd->comparehunk)
	{
		chd->comparehunk = ~0;
		if (read_hunk_into_memory(chd, _hunknum, chd->compare) == CHDERR_NONE)
			chd->comparehunk = _hunknum;
	}
	return (_hunknum == chd->comparehunk && !memcmp(rawdata, chd->compare, chd->header.hunkbytes));
}
