...
//-------------------------------------------------------------------------------
TBackendPluginManager::TBackendPluginManager(QObject *parent) :
    TAbstractPluginManager(parent)
{
    //Check and build type list.
    findPlugins("backend");
}

TBackendPlugins TBackendPluginManager::matchPluginBySuffix(QString suffix)
{
...
TBackendPlugin *TBackendPluginManager::parse(QString file, TMusicInfo *musicInfo)
{
    ...
    {
        TBackendPlugin *backendPlugin = (TBackendPlugin*)plugin;
        if(backendPlugin && backendPlugin->parse(pFile, musicInfo))
